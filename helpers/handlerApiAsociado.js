...
export default async function handlerApiAsociado({orderBy,wheres,callbackCreate,pageSize,page,id,req, res,coleccion,campoId,sinUsuario,buscaPorUsuario,limite}) {
                ...
                ejecuta=async ({user})=>{
                    ...
                    if(id){
                        ...
                        if(!campoId) salida=await findOne(coleccion,id)
                            ...
                            const datos=await findAll2({orderBy,wheres,coleccion,user:sinUsuario?user:null,sinUsuario,limite,page,pageSize})
