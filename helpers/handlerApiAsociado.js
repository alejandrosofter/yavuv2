...
export default async function handlerApiAsociado({
  orderBy,
  wheres,
  callbackCreate,
  pageSize,
      ...
      ejecuta = async ({ user }) => {
        ...
        if (id) {
          ...
          if (!campoId) salida = await findOne(coleccion, id);
            ...
            salida = await findOneField(coleccion, {
              campo: campoId ? campoId : "id",
              valor: id,
            });
        } else {
          const datos = await findAll2({
            orderBy,
            wheres,
            coleccion,
            user: sinUsuario ? user : null,
            sinUsuario,
            limite,
            page,
            pageSize,
          ...
          const cantidadRegistros = await cantidadColeccion2({
            coleccion,
            user,
            sinUsuario,
            limite,
