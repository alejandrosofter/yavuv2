...
export default function ItemsCobro({setFieldValue,values,campoItems="deudas",campoFormaPago="formasDePago"}){
    ...
    const cambiaItems=(items)=>{
        ...
        items.forEach(item=>{
            ...
            bonificacion+=Number(item.importeBonificacion?item.importeBonificacion:0)
    ...
    <TabsFormik label="Gral" vistas={[
        ...
        fnCambia={cambiaItems}
            ...
            dataModulo={[]} columnas={[
                ...
                { field: 'label_idProducto',headerName: 'Producto', width: 280,  },
                { field: 'importe',headerName: '$ Importe', width: 100,renderCell: (params) => { return formatMoney(params.value)}  },
                
                { field: 'importeBonificacion',headerName: '$ Bonif.', editable: false, width: 80,
                renderCell: (params) => { return formatMoney(params.value)}  },
                { field: 'importeTotal',headerName: '$ Total', width: 100,
                renderCell: (params) => { 
                    const bonif=Number(params.row.importeBonificacion)
                    const aux=(params.row.cantidad*params.row.importe)-(bonif?bonif:0)
        ...
        fnCambia={cambiaItemsFormaPago}
            ...
            textoEditar={`Puedes cambiar las propiedades:`}
            textoAgregar={`Ingrese los datos `}
            valoresIniciales={valoresInicialesFormaPago(importeDebe-importeHaber)} 
            form={<FormItemPago />} 
            dataModulo={[]} columnas={[
