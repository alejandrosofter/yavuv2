...
import {ModeloDisparadores,valoresInicialesDisapradores} from "../../modelos/ModeloDispositivo"
import FormItem from "./_formDisparadores"
export default function Form({mod,setFieldValue,values}){
    const tipoEquipos=mod.config.itemsTipoEquipos?mod.config.itemsTipoEquipos:[]
    return(
        <Grid >
            <Stack>
                    <Grid sx={{pt:1,pb:1}} md={12} container rowSpacing={2} spacing={2}>
                        ...
                        <Grid item md={2}><SelectFormik lista={tipoEquipos} campoId="id" campoLabel={"nombre"} label="Tipo Equipo" campo="tipoEquipo"/></Grid>
                        <Grid item md={2}><Input label="ID del equipo"  campo="identificador"/></Grid>
                        <Grid item md={2}><Input label="CLAVE del equipo"  campo="clave"/></Grid>
                        
                        <Grid item md={2}><SelectEstaticFormik items={["CONECTADO","PENDIENTE","DESCONECTADO"]}  label="Estado" campo="estado" /></Grid>
                        <Grid item md={12}>
                        <ItemsModulo
                            setFieldValue={setFieldValue} 
                            campo="disparadores" data={values.disparadores}
