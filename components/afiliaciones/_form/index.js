...
export default function Form({mod,setFieldValue,values}){
    ...
    return(
        <Grid sx={{pt:1,pb:1}} md={12} container rowSpacing={2} spacing={2}>
                <Grid item  md={2}><SelectFecha label="Fecha Alta" campo="fecha"/></Grid>
                <Grid item md={2}><SelectEstaticFormik items={["PENDIENTE","ALTA","BAJA"]}  label="ESTADO" campo="estado"/></Grid>
                        
                  
            <TabsFormik label="Datos Socio" vistas={[
            {label:"Socio",nro:0,vista:
    
                <FormSocios field="socio" setFieldValue={setFieldValue} values={values} mod={mod}/>
    
            },
            {label:"Debito Automatico",nro:1,vista:
            <Grid item md={12}>
           <FormDebitoAutomatico field="debitoAutomatico" setFieldValue={setFieldValue} values={values} mod={mod}/>
            </Grid>
            },
            {label:"Documentacion",nro:2,vista:
                <Grid item md={12}>
                <FormDocumentos setFieldValue={setFieldValue} values={values} mod={mod}/>
                </Grid>
            },
            {label:"Actividades",nro:3,vista:
            <Grid item md={12}>
                <FormActividades setFieldValue={setFieldValue} values={values} mod={mod}/>
            </Grid>
            },
            
            {label:"Familiares",nro:4,vista:
            <Grid item md={12}>
