...
export default function PerfilSocio({ mod }) {
  const router = useRouter();

  const [socioSeleccion, setSocioSeleccion] = React.useState(
  ...
  if (!dataSocio) {
    return (
      <div>
        ...
        <Typography>
          Sin socio seleccionado. Por favor busque un socio ...
        </Typography>
      </div>
    );
  }
        ...
        {dataSocio && (
          <Grid justifyContent="flex-end" container>
            <Grid item xs={9}>
              ...
              <ContainerTextoSocio>
                <Badge
                  color="secondary"
                  badgeContent={
                    dataSocio.estado === "ALTA" ? 0 : dataSocio.estado
                  }
                />
                ...
                <Link
                  passHref
                  href={`/mod/${router.query.id}/editar/${dataSocio.id}`}
                >
                ...
                <SubInfoSocio>
                  EN FLIA: {dataSocio.socioFlia?.label} |{dataSocio.domicilio} |
                  DNI:{dataSocio.dni} | {dataSocio.email}
                </SubInfoSocio>
              </ContainerTextoSocio>
            </Grid>
            <Grid item xs={3}>
            ...
            <Grid flex={1} item md={12}>
              <TabsSocio mod={mod} dataSocio={dataSocio} />
            </Grid>
          </Grid>
        )}
      </Grid>
