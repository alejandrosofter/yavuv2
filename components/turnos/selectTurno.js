...
import { Button, FormControl,Grid,Icon,InputLabel, Typography } from "@mui/material";
...
const SelectTurno = ({label,campo,callbackchange,consultorio,tipoTurno}) => {
    const [horarios,setHorarios]=useState([])
    const [fechaBusca,setFechaBusca]=useState(new Date(new Date().getFullYear(),new Date().getMonth(),new Date().getDay()+1) )
    ...
    console.log(fechaBusca)
        ...
        if(tipoTurno&&consultorio){
            const fecha="seconds" in fechaBusca?fechaBusca.seconds*1000:fechaBusca
            ...
            for(let i in consultorio.horarios){
                ...
                if(horario.tipoTurno===tipoTurno.id){
                    const CADA_MINUTOS=horario.duracion?horario.duracion:consultorio.duracion
...
<FormControl fullWidth>
  ...
  <Field label={label} name={campo} id={campo} >
    ...
    return( 
    <Grid container>
        <Typography variant="caption" style={{marginTop:-5}}>{label}</Typography>
        <Grid item md={1}>
            <Button disabled={!(consultorio&&tipoTurno)} onClick={buscaTurno}>
            {!props.field.value &&<Icon className="fas fa-search"/> }
        {props.field.value?getFechaString(props.field.value):"seleccione"}</Button></Grid>
        <DialogContenido fullWidth={true} maxWidth="sm" open={openTurnosDisponibles} setOpen={setOpenTurnosDisponibles} titulo="Turnos DISPONIBLES">
                            <Grid container>
                            <Grid item md={3}>  <SelectFechaSimple callbackChange={cambiaFecha}/> </Grid>
                            <Grid sx={{pl:2}} item md={2}><Button onClick={buscarTurnos}><Icon className="fas fa-search"/> Buscar
                                </Button></Grid>
                                <Grid item md={12}>
                                    <ListaSimple onClick={handleChange} items={horarios} campoId="value" fnRender={item=>item.disabled?
