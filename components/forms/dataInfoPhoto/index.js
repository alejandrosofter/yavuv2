...
export default function Modulo({fnCambia}){
    ...
    const data_dorsoDni=[
        {labelPrev:["DOMICILIO:","DOMICILIO"],labelField:"Domicilio",posicionesPosFind:7,field:"domicilio"},
        {labelPrev:["CUIL:","CUIL"],labelField:"CUIL",posicionesPosFind:1,field:"cuil"},
    ]
    const getMes=(valor)=>{
    ...
    const data_frenteDni=[
        {labelPrev:"Name",posicionesPosFind:2,field:"nombre",labelField:"Nombre"},
        {labelPrev:["Surname","Surneme"],posicionesPosFind:1,field:"apellido",labelField:"Apellido"},
        {labelPrev:"Sex",posicionesPosFind:1,labelField:"Sexo",field:"sexo",fn:(valor)=>valor=="M"?"MASCULINO":"FEMENINO"},
        {labelPrev:"birth",posicionesPosFind:4,labelField:"Fecha Nac.",field:"fechaNacimiento",fn:(valor)=>{
        ...
        {labelPrev:"Document",posicionesPosFind:1,labelField:"D.N.I",field:"dni",fn:(valor)=>valor.replaceAll(".",'')},
    ]
    ...
    const getData=(arrGoogle,arrDataBusco)=>{
        ...
        const res=arrDataBusco.map(item=>{
            const i=getIndexItemArray({data:arrData,valor:item.labelPrev,campoId:campoValor})
           
            if(i>0)return {
                value:getValorBuscado(arrData,i,item.posicionesPosFind,campoValor,item?.fn),
                campo:item.field,
...
const colsTabla=[{field:"value",label:"Valor",fn:(valor)=>{
    if(moment.isDate(valor))return getFechaString(valor)
    return valor
}},{label:"Campo",field:"labelCampo"}]
const listaSelect=[{label:"Frente DNI",value:"frenteDni",id:"1"},{id:"2",label:"Dorso DNI",value:"dorsoDni"}]
    return(
        <Stack sx={{}}>
            <ImageUpload folder={folder} callbackchange={doneUpload} icon={<Icon className="fas fa-barcode"/>}  />
            <DialogContenido titulo="RESULTADOS SCAN" open={openResultados} setOpen={setOpenResultados}>
                <SelectSimple label="Forma" lista={listaSelect} fn={cambiaSelect}/>
                <Tabla cols={colsTabla} data={dataResultados}/>
                <Button onClick={clickAceptar}><Icon className="fas fa-tick"/>ACEPTAR</Button>
            </DialogContenido>
        </Stack>
