  ...
  render() {
    return (
        ...
        <Field label={this.props.label} name={this.props.campo} id={this.props.campo} >
          {(props) =>{
              useEffect(() => {
                this.setState({filesUrls:props.form.values.logo?props.form.values.logo:[]})
              ...
              const quitarArr=(urlRef)=> {
                const i=this.state.filesUrls.map(item=>item.urlRef).indexOf(urlRef)
            ...
            const clickQuitar=async (e)=>{
                ...
                if(elem){
                    Storage().ref().child(elem.value).getDownloadURL().then(async (err)=>{
                        console.log("existe")
