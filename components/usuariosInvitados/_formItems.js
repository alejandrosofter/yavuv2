  ...
  const cambiaMod=(valor,item)=>{
    ...
    return(
            <Grid spacing={2} container>
                <Grid item md={12}><SelectFormik label="Modulo " callbackchange={cambiaMod} lista={mods} campoLabel="label" campoId="id" campo="idMod"/></Grid>
                <Grid item md={3}><SwitchFormik label="Habilitado " campo="habilitado"/></Grid>
                <Grid item md={2}><SwitchFormik campo='accesoTotal' label="Acceso Total" /></Grid>
                {recursos && <Grid item md={6}><SelectFormik  multiple={true} lista={recursos} campoId="id"
                 campoLabel={modSeleccion.labelField} campo='recursos' label="Recursos" /></Grid>}
