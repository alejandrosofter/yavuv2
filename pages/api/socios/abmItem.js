import {findOneField,updateArrayField, findOne,update, nuevo} from "../../../config/firebase";
...
const fncallback=async (req)=>{
    ...
    const fn={
        actividades:async (data,esDelete)=>{
            const actividad=await findOne(subColeccion,data.idActividad?data.idActividad:data.id.idActividad)
    
            const periodos=(actividad.periodos)?actividad.periodos:[]
            
            const indexPeriodo=periodos.map(item=>item.id).indexOf(data.idPeriodo?data.idPeriodo:data.id.idPeriodo)
            let periodoAux=periodos[indexPeriodo]
           
            if(periodoAux){
                let sociosPeriodo=periodoAux.socios?periodoAux.socios:[]
                if(esDelete) sociosPeriodo=quitarArr({arr:sociosPeriodo,valor:idRegistroPadre,campoId:"id"})
                else sociosPeriodo=agregarArr({campoId:"id",arr:sociosPeriodo,valor:{id:idRegistroPadre,nombre:label,idActividad:data.idActividad,idSubActividad:data.idSubActividad}})
