...
const getTemplate = async (idPantilla) => {
  const data = await findOne("plantillas", idPantilla);
  if (!data) return "s/n";
  return data.dataPlantilla;
};
export default async function handler(req, res) {
  const { id, data } = req.body;
  console.log("idPantilla", id);
  const str = await getTemplate(id);
  const strRes = await remplazarValores(str, data);
