...
export default function Modulo({ mod, callbackSuccess }) {
  ...
  const [consultorio, setConsultorio] = useState(
    localStorage.getItem("consultorio")
      ? localStorage.getItem("consultorio")
      : ""
  );
  const { data } = useCollection("turnos", {
    listen: true,
    where: [
  ...
  const turnosCalendario = data
    ? data.map((item) => ({
        ...item,
        title: `${item.label_paciente} - ${item.detalle}`,
        start: new Date(item.fechaTurno.seconds * 1000),
        end: new Date(
          new Date(item.fechaTurno.seconds * 1000).setMinutes(
            new Date(item.fechaTurno.seconds * 1000).getMinutes() +
    ...
    <Grid spacing={2} container>
      ...
      <Grid item md={12}>
        ...
        <Calendario
          ...
          onSelectEvent={cambiaTurno}
          eventos={turnosCalendario}
        />{" "}
      </Grid>
      <Dialog
        fullWidth={true}
      ...
      >
        <EditarTurno
          idItem={turnoSeleccion?.id}
          mod={mod}
          callbackSuccess={() => setShowEdit(false)}
        />
      </Dialog>
      <Dialog
        fullWidth={true}
      ...
      >
        <NuevoTurno
          preData={preDataTurno}
          mod={mod}
          callbackSuccess={() => setShowNew(false)}
        />
      </Dialog>
    </Grid>
  );
