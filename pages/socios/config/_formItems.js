...
export default function FormItemConfigSocios({ values }) {
  ...
  return (
    <Grid container spacing={2}>
      ...
      <Grid item md={3}>
        <SwitchFormik
          callbackChange={cambiaGeneraDeuda}
          campo="generaDeuda"
          label="Genera Deuda"
        />
      </Grid>
      <Grid sx={{ display: visibleConfig, p: 2 }} container spacing={2}>
        <Grid item md={8}>
          <Input campo="aplicaDeudaConjunto" label="Aplica a ..." />
        </Grid>

        <Grid item md={12}>
          <Input campo="calculoImporte" label="Calculo Importe" />
        </Grid>
        <Grid item md={12}>
          <Input campo="fnDetalleConcepto" label="Fn Detalle Concepto" />
        </Grid>
        <Grid item md={12}>
          <Input campo="fnDetalleExtra" label="Fn Detalle extra" />
        </Grid>
        <Grid item md={12}>
          <Input label="Fn Label elemento" campo="fnLabelElemento" />
        </Grid>
      </Grid>
      <Stack direction="row" spacing={2}>
        <Typography sx={{ color: "#31877fde", pl: 4 }}>
          Variables usables:{" "}
          {
            "${nombreActividad}${actividad},${calculoTipoSocio},${nombre}, ${apellido},${sociosActivos}, ${edad}, ${tipoSocio}"
          }
        </Typography>
      </Stack>
    </Grid>
