...
export default function ModeloAfiliados(){
    return yup.object().shape({
        ...
        socio: yup.object().shape({
            ...
            dni:yup.string().required("Es necesario el DNI").test(
                'Dni no existente',
                '${path} EXISTENTE',
                async (value, testContext) => {
                    const res=await(await fetch(`/api/validadores/socio/dni/${value}`)).json()
                    //responseText === 'true'
                    
                    if("apellido" in res)
                        return testContext.createError({message:`${res.apellido.toUpperCase()} ${res.nombre.toUpperCase()} registrado`})
